...
#define URL_VERB_DEFAULT "upload"
#define URL_VERB_ALT     "live"

int main(int argc,char *argv[])
{
    int rc = 0; /* default success */
    int alt = 0; /* default default */

    if (argc < 4) { puts("ERROR Parameters: <host> <port> <seqn> [<alt>]"); exit(1); }
    ...
    rc = snprintf(arg_url_path, 255, "/api/%s/%s", 
                                alt==0?URL_VERB_DEFAULT:URL_VERB_ALT, argv[3]);
    if ( rc < 0 ) { perror("ERROR /api/<verb>/path failed"); exit(1); }
    else if ( rc >= 255 ) { puts("ERROR /api/<verb>/path failed truncated"); exit(1); }
    ...
    int failed = 0;
    do {
        rc = snprintf(message, message_size - 1, 
    ...
    if (failed == 0) {
        ...
        /* HTTP/1.0 200 OK */
        do {
            if (strnlen(response, 4095) >= 4094 ) { break; }
            response[sizeof(response)-1] = 0;
            char *cp1 = strstr(response, "HTTP/");
            if ( cp1 == NULL ) break;
            char *cp2 = strstr(response, "\r\n");
            if ( cp2 == NULL ) break;
            *cp2 = '\0'; /* terminate at the first line */
            char *cp3 = strstr(response, "200");
            if (cp3 == NULL ) break;
            if ( cp1 < cp3 && cp3 < cp2 ) {
                printf("ResponseOk found\n");
                return 0; /* found 200 */
            }
        } while(0);
    }

    return 1; /* not searching for 200 */
}
